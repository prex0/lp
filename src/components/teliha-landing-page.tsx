...
import { Button } from "@/components/ui/teliha-button"
import ReactFlow, {
  Node,
  Edge,
  addEdge,
...
import { Shuffle, TrendingUp, FileText, Cog, FileCode, Repeat, Database, BarChart2, LineChart, Droplet, PieChart, Wallet, Key, Twitter, Building, MapPin, Code, Globe, Users } from "lucide-react"
      ...
      data: { 
        label: (
          <div className="flex items-center">
            <PersonIcon className="mr-2" size={18} />
            <span>Lender</span>
          </div>
        ) 
      },
...
const TeamMember: React.FC<{ 
  ...
  <div className="flex flex-col items-center text-center">
    <img src={image} alt={name} width={96} height={96} className="rounded-full mb-2" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
            <div>
              ...
              <p className="mt-4 text-sm text-gray-600">
                Predy Protocolは、分散型金融（DeFi）の革新的なソリューションです。このプロトコルは、レンダー、トレーダー、ファシリテーター間の複雑な相互作用を可能にし、効率的な資金の流れと取引の実行を実現します。スマートコントラクトを活用することで、安全かつ透明性の高い取引環境を提供します。
              </p>
            </div>
            <div>
              <a 
              ...
              <p className="mt-4 text-sm text-gray-600">
                Prexは、次世代のSaaSソリューションとして設計されています。従来のEOA（外部所有アカウント）とAA（抽象アカウント）の両方をサポートし、ユーザーに柔軟性を提供します。インテントオーダー、ファシリテーター、ハンドリングコントラクトを通じて、効率的で安全な取引プロセスを実現し、DeFiの利用をより簡単かつアクセスしやすいものにします。
              </p>
            </div>
          </div>
        </div>
